...
function captureHandler(event) {
    ...
    newBox.style.left = `${event.pageX}px`;
    newBox.style.top = `${event.pageY}px`;
    actionArea.appendChild(newBox);

    actionArea.addEventListener('pointermove', (event) => {
        newBox.style.left = `${event.pageX}px`;
        newBox.style.top = `${event.pageY}px`;
    });
}

duplicator.addEventListener('pointerdown', captureHandler);
